...
export function HomeScreen() {
  ...
  const t = useT();
  const gardenPoints = useStore((s) => s.gardenPoints);
  const streak = useStore((s) => s.streak);
  const isPremium = useStore((s) => s.isPremium);
  const resets = useStore((s) => s.resets);

  const { resetsToday, hoursSince } = useMemo(() => {
    ...
    const todayCount = completed.filter((r) => startOfDay(r.at) === today).length;
    const hours =
      completed.length === 0
        ? 99
        : (Date.now() - completed[completed.length - 1].at) / (60 * 60 * 1000);
    return { resetsToday: todayCount, hoursSince: hours };
  }, [resets]);
